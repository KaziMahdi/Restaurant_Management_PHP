...
class Purchase implements JsonSerializable{
	...
	public function save(){
		global $db,$tx;
		// $sql="insert into {$tx}purchases(supplier_id,purchase_date,delivery_date,shipping_address,purchase_total,paid_amount,remark,status_id,discount,vat)values('$this->supplier_id','$this->purchase_date','$this->delivery_date','$this->shipping_address','$this->purchase_total','$this->paid_amount','$this->remark','$this->status_id','$this->discount','$this->vat')";
		// $db->query($sql);
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		$top = ($page - 1)*$perpage;
		$result=$db->query("select p.id,s.name,p.purchase_date,p.delivery_date,p.shipping_address,p.purchase_total,p.paid_amount,p.remark,p.status_id,p.discount,p.vat from {$tx}purchases p,{$tx}suppliers s where p.supplier_id=s.id $criteria limit $top,$perpage");
		$html="<table class='table'>";
			$html.="<tr><th colspan='3'><a class=\"btn btn-success\" href=\"create-purchase\">New Purchase</a></th></tr>";
		if($action){
			$html.="<tr><th>Id</th><th>Supplier Name</th><th>Purchase Date</th><th>Delivery Date</th><th>Shipping Address</th><th>Purchase Total</th><th>Paid Amount</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Supplier Name</th><th>Purchase Date</th><th>Delivery Date</th><th>Shipping Address</th><th>Purchase Total</th><th>Paid Amount</th></tr>";
		}
		while($purchase=$result->fetch_object()){
			$action_buttons = "";
			if($action){
				$action_buttons = "<td><div class='clearfix' style='display:flex;'>";
				$action_buttons.= action_button(["id"=>$purchase->id, "name"=>"Details", "value"=>"Detials", "class"=>"info", "url"=>"details-purchase"]);
			...
			$html.="<tr><td>$purchase->id</td><td>$purchase->name</td><td>$purchase->purchase_date</td><td>$purchase->delivery_date</td><td>$purchase->shipping_address</td><td>$purchase->purchase_total</td><td>$purchase->paid_amount</td> $action_buttons</tr>";
		}
		$html.="</table>";
